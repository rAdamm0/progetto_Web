<section class="border bg-dark rounded p-4">
    <p class="text-start text-white">Benvenuta matricola su WebLio, la prima biblioteca totalmente online.
        Qui puoi trovare qualsiasi libro utile per il tuo processo di studio</p>
    <button type=" button" class="btn btn-light">Più informazioni</button>
</section>
<section>
    <img src="../../html/assets/foto.png" alt="" class="img-fluid container-md align-items-center">
</section>
<section class="container my-5 bg-dark p-5 rounded">
    <h2 class="mb-4 fw-bold text-white">Cosa Forniamo</h2>
    <div class="row g-4">
        <!-- Card 1 -->
        <div class="col-12 col-md-4">
            <div class="card h-100">
                <img src="../html/assets/foto.png" class="card-img-top" alt="Profilo">
                <div class="card-body">
                    <h5 class="card-title"><a href="personal.php">Profilo</a></h5>
                    <p class="card-text">
                        Puoi creare un semplice profilo immettendo solamente la tua matricola e password!
                    </p>
                </div>
            </div>
        </div>

        <!-- Card 2 -->
        <div class="col-12 col-md-4">
            <div class="card h-100">
                <img src="../html/assets/foto.png" class="card-img-top" alt="Catalogo">
                <div class="card-body">
                    <h5 class="card-title"><a href="catalogo.php">Catalogo</a></h5>
                    <p class="card-text">
                        Un Catalogo curato e pieno di libri di ogni tipo e categoria!
                    </p>
                </div>
            </div>
        </div>

        <!-- Card 3 -->
        <div class="col-12 col-md-4">
            <div class="card h-100">
                <img src="../html/assets/foto.png" class="card-img-top" alt="Prenotazioni">
                <div class="card-body">
                    <h5 class="card-title"><a href="prenotazioni.php">Prenotazioni</a></h5>
                    <p class="card-text">
                        Prenota quello che necessiti completamente online e ti verrà fornito direttamente tramite email.
                    </p>
                </div>
            </div>
        </div>
    </div>
    <div class="container my-5 text-white">
        <div class="row align-items-center">

            <!-- Colonna sinistra (testo) -->
            <div class="col-12 col-md-6">
                <h2 class="fw-bold">Come Prenoto?</h2>

                <h5>Sfoglia il catalogo</h5>
                <p>Sfoglia il catalogo e trova uno o più libri che ti possano interessare</p>

                <h5>Prenota il libro</h5>
                <p>Visualizza la disponibilità e prenota il libro se possibile</p>

                <h5>Consulta la pagina Prenotazioni</h5>
                <p>In questa pagina troverai un calendario con tutti i libri prenotati e la possibilità di prenotarli nuovamente</p>

                <button class="btn btn-light"><a href="catalogo.php">Catalogo</a></button>
                <button class="btn btn-light"><a href="book.php?id=<?php echo rand($templateParams["range"]["min"], $templateParams["range"]["max"])?>" >Libro Fortunato</a></button>
            </div>

            <!-- Colonna destra (immagine) -->
            <div class="col-12 col-md-6">
                <img src="../html/assets/foto.png" class="img-fluid rounded-3" alt="">
            </div>
        </div>
    </div>
</section>
<section>
    <div class="container text-center">
        <h2 class="mb-5">Chi Siamo?</h2>

        <div class="row justify-content-center pb-5">

            <!-- Profilo 1 -->
            <div class="col-12 col-md-5">
                <img src="../html/assets/foto.png" class="rounded w-75 img-fluid mb-3" alt="...">
                <h5>Lorenzo Casadei</h5>
                <p>matr:</p>
            </div>

            <!-- Profilo 2 -->
            <div class="col-12 col-md-5">
                <img src="../html/assets/foto.png" class="rounded w-75 img-fluid mb-3" alt="...">
                <h5>Adam Paolo Razzino</h5>
                <p>matr:</p>
            </div>

        </div>
    </div>
</section>
